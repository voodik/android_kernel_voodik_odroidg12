...
static int do_fe_set_property(struct file *file, unsigned int cmd,
		struct compat_dtv_properties __user *dtv32)
{
	...
	err = do_ioctl(file, FE_SET_PROPERTY, (unsigned long) dtv);
...
static int do_fe_get_property(struct file *file, unsigned int cmd,
		struct compat_dtv_properties __user *dtv32)
{
	struct dtv_properties __user *dtv;
	struct dtv_property __user *properties;
	...
	err = do_ioctl(file, FE_GET_PROPERTY, (unsigned long) dtv);
		 ...
		 unsigned long arg, struct file *file)
{
	...
	case FE_SET_PROPERTY32:
		return do_fe_set_property(file, cmd, argp);
	case FE_GET_PROPERTY32:
		return do_fe_get_property(file, cmd, argp);
#endif
	}
