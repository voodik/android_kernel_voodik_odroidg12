...
#define PATCH_V3_SECTION_PATCH_IMAGE		0x01

#define DL_FIX_PATCH_ADDR	0
#define DL_FIX_SEC_HDR_ADDR	1
#define DL_FIX_ADDR_MAX		2
...
struct rtl_sec_hdr_v3 {
	...
	u64 patch_start_addr;
	u64 patch_start_value;
...
/* { pid, lmp_sub, mp_fw_name, fw_name, config_name, chip_type } */
	...
	{0x1bef, 0x8761, "mp_rtl8761cu_fw", "rtl8761cu_mx_fw", "rtl8761cu_mx_config", RTL8761CU}, /* RTL8761CU-ASUS*/
...
static void btrtl_insert_ordered_subsec_v3(struct rtl_subsection_v3 *node)
{
	...
	list_for_each_safe(pos, next, &list_secs) {
		subsec = list_entry(pos, struct rtl_subsection_v3, list);
		if (subsec->index >= node->index)
			break;
	}
...
static struct rtl_subsection_v3 *btrtl_reverse_search_subsec(void)
{
	...
	list_for_each_entry_reverse(sec, &list_secs, list) {
		RTKBT_DBG("Addr:0x%08x, Value: 0x%08x",
			  sec->imem_addr[1], sec->imem_value[1]);
		/* Check sec hdr in section */
		if (!sec->imem_value[1] || sec->imem_addr[1] == 0x00 ||
		    sec->imem_addr[1] == 0xffffffff)
			continue;
		return sec;
	}
...
static int download_patch_sections(dev_data *dev_entry, xchange_data *xdata)
{
	...
	u8 i;
	int patch_size = 0;
	int ret_val = -1, config_len = 0;
	u8 *config_file_buf = NULL;

	config_file_buf = load_config(dev_entry, &config_len);
	list_for_each_entry_safe(sec, tmp, &list_secs, list) {
		RTKBT_DBG("RTL: index %02x", sec->index);

		for (i = 0; i < 2; i++) {
			if (sec->imem_addr[i] == 0 ||
			    sec->imem_addr[i] == 0xFFFFFFFF) {
				RTKBT_WARN("%s: no need to write addr 0x%08x",
					   __func__, sec->imem_addr[i]);
				continue;
			}
			RTKBT_DBG("%s: write Addr and Val, 0x%08x, 0x%08x",
				  __func__, sec->imem_addr[i],
				  sec->imem_value[i]);
			if (btrtl_vendor_write_mem(dev_entry, sec->imem_addr[i],
						   sec->imem_value[i])) {
				RTKBT_ERR("PATCH V3:  Write Reg failed");
				goto exit_fail;
			}
			continue;
		}

		xdata->fw_len = sec->patch_image_len;
		...
		if (&tmp->list == &list_secs) {
			RTKBT_DBG("RTL: index %02x, Add config file",
				  sec->index);
			if (config_len)
				memcpy(buf + sec->patch_image_len,
				       config_file_buf, config_len);
		}
		fw_data = xdata->fw_data;
		xdata->fw_data = buf;

		RTKBT_DBG("PATCH V3: patch image (%d). len: %d",
			  sec->index, xdata->fw_len);
...
static int btrtl_finalize_download(xchange_data *xdata)
{
	int ret = 0;
	...
	RTKBT_DBG("PATCH V3: btrtl_finalize_download");

	xdata->cmd_hdr->opcode = cpu_to_le16(DOWNLOAD_OPCODE);
	xdata->cmd_hdr->plen = sizeof(u8);
	cmd_para->index = 0x80;
	xdata->pkt_len = sizeof(*xdata->cmd_hdr) + 1;
...
static int rtl_download_firmware_v3(dev_data * dev_entry, xchange_data *xdata)
{
	...
	if (subsec) {
		RTKBT_DBG("RTL: index %02x", subsec->index);
		imem_value = subsec->imem_value[DL_FIX_PATCH_ADDR] +
			     subsec->patch_image_len -
			     subsec->imem_value[DL_FIX_SEC_HDR_ADDR];
		if (btrtl_vendor_write_mem(dev_entry,
					   subsec->imem_addr[DL_FIX_PATCH_ADDR],
					   imem_value)) {
			RTKBT_ERR("PATCH V3:  Write Reg failed");
			goto exit_fail;
		}
	...
	/* Download the final pkt to complete the download procedure */
	ret = btrtl_finalize_download(xdata);
	if(ret < 0)
		goto exit_fail;

	RTKBT_DBG("PATCH V3:  Firmware Download Successful");
	return ret;

exit_fail:
	RTKBT_DBG("PATCH V3:  Firmware Download Failed");
	return ret;
}
...
static int rtlbt_parse_section_v3(u32 opcode, u8 *data, u32 len)
{
	...
	if (!sec_hdr) {
		RTKBT_DBG("get section header failed");
		return -1;
	}

	sec_len = le64_to_cpu(sec_hdr->patch_image_len);

	RTKBT_DBG("sec, ic cut 0x%02x, len %08x", sec_hdr->ic_cut, sec_len);
	...
	subsec->imem_addr[DL_FIX_PATCH_ADDR] = le64_to_cpu(sec_hdr->patch_start_addr);
	subsec->imem_value[DL_FIX_PATCH_ADDR] = le64_to_cpu(sec_hdr->patch_start_value);

	subsec->imem_addr[DL_FIX_SEC_HDR_ADDR] = le64_to_cpu(sec_hdr->sec_hdr_addr);
	subsec->imem_value[DL_FIX_SEC_HDR_ADDR]  = le64_to_cpu(sec_hdr->sec_hdr_value);
	subsec->patch_image_len = (u32)le64_to_cpu(sec_hdr->patch_image_len);
	subsec->patch_image_buf = ptr;
	subsec->image_ver = get_unaligned_le32(ptr + subsec->patch_image_len - 4);
	RTKBT_DBG("sub section image version: %08x", subsec->image_ver);
				   ...
				   int fw_size)
{
	...
	key_id = rtk_vendor_read(dev_entry, READ_SEC_PROJ);
	if (key_id < 0)
		return -EIO;

	RTKBT_DBG("%s: key id %u", __func__, key_id);
	...
	RTKBT_DBG("FW version %08x-%08x, num:%u", *((u32 *) hdr->fw_version),
		  *((u32 *) (hdr->fw_version + 4)), num_sections);
	...
	for (i = 0; i < num_sections; i++) {
		...
		switch (opcode) {
		case PATCH_V3_SECTION_PATCH_IMAGE:
...
int load_firmware(dev_data *dev_entry, xchange_data *xdata)
{
	...
	if (!memcmp(epatch_buf, RTK_EPATCH_SIGNATURE_V3, 8)) {
		...
		buf_len = rtlbt_parse_firmware_v3(dev_entry, epatch_buf,
						  fw->size);
		xdata->multi_image = true;
		xdata->multi_data = epatch_buf;
		goto done;
	}
...
alloc_fail:
	...
	if (epatch_buf && !xdata->multi_image)
		vfree(epatch_buf);
...
get_ver:
	...
	if (xdata->multi_image) {
		list_for_each_entry_safe(sec, tmp, &list_secs, list) {
			RTKBT_DBG("RTL: index %02x, %04x, %04x", sec->index,
				  (u16) (sec->image_ver >> 16),
				  (u16) sec->image_ver);
			if ((u16) (sec->image_ver >> 16) == hci_rev &&
			    ((u16) sec->image_ver & lmp_subver))
				return 1;
			else
				return -1;
		}
...
{
	struct rtl_subsection_v3 *sec, *tmp;
	u32 sec_hdr_size_value, imem_value;
	u8 reg_val[4];
	int ret_val = -1;
	int retry = 0;
...
get_ver:
	list_for_each_entry_safe(sec, tmp, &list_secs, list) {
		...
		if (!sec->index) {
			ret_val = check_fw_version(xdata);
			if (ret_val < 0) {
				RTKBT_DBG("RTL: check version failed index %02x", sec->index);
				goto version_end;
			}

		} else {
			RTKBT_DBG("%s: write Addr and Val, 0x%08x, 0x%08x",
				  __func__, sec->imem_addr[DL_FIX_PATCH_ADDR],
				  sec->imem_value[DL_FIX_PATCH_ADDR]);

			sec_hdr_size_value = sec->imem_value[DL_FIX_SEC_HDR_ADDR];
			if (sec->imem_addr[DL_FIX_SEC_HDR_ADDR] == 0 ||
			    sec->imem_addr[DL_FIX_SEC_HDR_ADDR] == 0xFFFFFFFF)
				sec_hdr_size_value = 0;

			imem_value = sec->imem_value[DL_FIX_PATCH_ADDR] +
					sec->patch_image_len -
					sec_hdr_size_value - 4;
			ret_val = rtk_vendor_read_reg32(dev_entry, imem_value, reg_val);
			if (ret_val <= 0) {
				RTKBT_ERR("PATCH V3:  Write Reg failed");
				goto version_end;
			}
				 ...
				 u8 *param, u32 timeout)
{
	...
	init_xdata(xdata, &dev_entry);
	if (!xdata->send_pkt || !xdata->rcv_pkt) {
		RTKBT_ERR("allocate send_pkt/rcv_pkt error");
		ret = -ENOMEM;
		goto end;
	}
	...
	if (ret < 0) {
		RTKBT_ERR("Failed to send HCI cmd %04x", opcode);
		goto end;
	}
